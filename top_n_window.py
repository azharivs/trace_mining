...
last_ts = [None, None, None, None]
wnd_start = [None, None, None, None]
# sample window length in nanoseconds
wnd_length = 1000000000 
...
for event in col.events:
    ...
    if last_ts[cpu_id] is None:
        # we start here
        last_ts[cpu_id] = cur_ts
        wnd_start[cpu_id] = cur_ts
    
    # previous task command (short) name
    prev_comm = event['prev_comm']+'_cpu_'+str(cpu_id)
    ...
    if cur_ts - wnd_start[cpu_id] >= wnd_length:
        wnd_start[cpu_id] = cur_ts
        # print top 5
        print('******************')
        rank=0;
        for name, ns in exec_times.most_common(10):
            rank += 1
            s = ns / 1000000000
            print(rank,'{:30}{} s'.format(name, s))
        print(sum(exec_times.values())/1000000000)
        exec_times.clear()
