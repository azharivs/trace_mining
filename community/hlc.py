...
class HierarchicalLinkClusteringApp(object):
    ...
    def __init__(self):
        ...
        self.parser.add_option("-t", "--threshold", metavar="THRESHOLD",
                ...
                help="use the given THRESHOLD to cut the dendrogram. "
                     ...
                     "automatically.")
        self.parser.add_option("-m", "--weight-min", metavar="WEIGHT_THRESHOLD_MIN",
                dest="weight_threshold_min", type=int, default=0,
                help="remove edges with weight strictly below the given WEIGHT_THRESHOLD_MIN. "
                     "Default = 0.")
        self.parser.add_option("-M", "--weight-max", metavar="WEIGHT_THRESHOLD_MAX",
                dest="weight_threshold_max", type=int, default=10000000,
                help="remove edges with weight strictly above the given WEIGHT_THRESHOLD_MAX. "
                     "Default = 0.")
    ...
    def process_file(self, filename):
        ...
        #delete edges with weight below/above some threshold provided as input
        if "weight" in graph.edge_attributes(): 
            ...
            graph.es(weight_lt=self.options.weight_threshold_min).delete()
            graph.es(weight_gt=self.options.weight_threshold_max).delete()
        ...
        # Print the results
        i = 0
        pal = drawing.colors.ClusterColoringPalette(n=100)
