    ...
    if os.path.exists(pklfile): #there is a corpus file for this trace so read from it
        ...
        for tid in tids:
            ...
            for i in range(2,len(event_list)):
                if not ((last_event in stopwords) or last_event[0:3]=='irq'): second_last_event = last_event #TODO: I am assuming it never happens that the first event is in stopwords
                if not ((event in stopwords) or event[0:3]=='irq'): last_event = event #TODO: I am assuming it never happens that the first event is in stopwords
                event = event_list[i]
                if (event in stopwords) or (event[0:3]=='irq'): continue #ignore stop words
