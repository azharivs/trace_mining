...
class SNNClusteringApp(object):
    ...
    def __init__(self):
        ...
        self.parser.add_option("-E", "--eps", metavar="EPS",
                dest="Eps", type=int, default = 2, 
                help="Threshold Eps similarity (shared NNs) value, below which is considered as noise. Default = 2.")
    ...
    def process_file(self, filename):
        ...
        for i in range(Vsize[0]):
            ...
            knn[i][nn] = 1;
        # now knn is an np.array for which knn[i][j]==1 iff i is among the k-nearest neighbors of j    
        
        # Next we compute the shared nearest neighbor similarity matrix
        # however, since DBSCAN requires this to be a distance matrix then
        # we make all the diagonals zero and take the inverse of the number of shared neighbors 
        # snn[i][j] will hold the number of common k-nearest neighbors of i and j
        snn = np.zeros(Vsize)
        ...
        siml = sorted(siml,reverse=False)
        print("shared NN=", np.unique(siml))
        points = []
        ...
        # Compute DBSCAN
        db = DBSCAN(eps=1/self.options.Eps, min_samples=self.options.MinPts, metric='precomputed').fit(snn)
        core_samples_mask = np.zeros_like(db.labels_, dtype=bool)
        core_samples_mask[db.core_sample_indices_] = True
        graph.vs["cluster"] = db.labels_
        ...
        # Print the results
        pal = drawing.colors.ClusterColoringPalette(n=n_clusters_+2)
        graph.vs["color"] = 'black' #color of noise
        
        #core = [i for i in range(Vsize[0]) if db.labels_[i] > -1]
        graph.vs[list(db.core_sample_indices_)]["color"] = pal.get(list(db.labels_[list(db.core_sample_indices_)])) #[pal.get(i) for i in db.labels_ if i > -1]
        graph.es["color"] = 'blue'
        noise = [graph.vs[i]["name"] for i in range(Vsize[0]) if db.labels_[i] == -1]
        print(n_clusters_," Clusters")
        print(len(db.core_sample_indices_)," core events=",graph.vs[list(db.core_sample_indices_)]["name"])
        print(len(noise), " noise events=",noise)
        print("total events = ", Vsize[0])
        ...
        ctx.fill()
        plt = plot(graph, vertex_shape = 'circle', bbox = (width, height), layout = layout, palette = pal)
        #plt.background = None
        #plt.redraw()
